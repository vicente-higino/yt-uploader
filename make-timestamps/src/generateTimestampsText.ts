    ...
    if (categoriesArray.length > 0) {
        ...
        for (let i = 1; i < categoriesArray.length; i++) {
            ...
            if (!isValidDate(currentCategory.startTimestamp) || !isValidDate(lastMergedCategory.startTimestamp)) {
                console.error(
                    `Invalid startTimestamp found during merge check. Current: ${
                        JSON.stringify(currentCategory)
                    }, Last Merged: ${JSON.stringify(lastMergedCategory)}`,
                );
            ...
            if (
                ...
                lastMergedCategory.startTimestamp = new Date(Math.min(lastMergedCategory.startTimestamp.getTime(), currentCategory.startTimestamp.getTime())); // Retain the earlier timestamp during merge
